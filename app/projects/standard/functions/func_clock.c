...
void func_clock_process(void)
{
    ...
    if (RTCCON10 & BIT(0)) {
        ...
        if (!tm_cb.setting) {
            rtc_time_to_tm(RTCCNT, &rtc_tm);            //更新时间结构体
            RTCALM = rtc_tm_to_time(&rtc_tm) + (CLARM_TIME - 1);       //设置闹钟相对于当前时间5秒后
...
static void func_clock_enter(void)
{
    ...
    u32 rc2m_clk = rc2m_clk_nhz_get(0);
    rtc_prescale_set(50835-1);
    rtc_counter_set(0);

    rtc_time_to_tm(RTCCNT, &rtc_tm);        //更新时间结构体
    rtc_set_alarm_wakeup(CLARM_TIME-1);                //设置闹钟相对于当前时间5秒后
    func_clock_enter_display();
#if WARNING_FUNC_CLOCK
    mp3_res_play(RES_BUF_CLOCK_MODE_MP3, RES_LEN_CLOCK_MODE_MP3, 0);
#endif // WARNING_FUNC_CLOCK
}
    printf("rtc_prescale_get %d \n",rtc_prescale_get());
    // rtc_prescale_set(51535-1);
