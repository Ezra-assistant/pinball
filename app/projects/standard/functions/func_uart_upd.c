...
void uart_upd_init(u32 uart_port_map, u32 uart_baud, u8 sys_clk)
{
    TRACE("uart_upd_init\n");
    u32 baud = 0;
    baud = ((SYS_CLK + (uart_baud / 2)) / uart_baud) - 1;
...
#if UART_S_UPDATE_CAPTRUE
const u8 upd_s_start_sign[] = { 'S', 'T', 'A', 'R', 'T', '_', 'U', 'P', 'D', '^', '_', '^' };
u32 upd_s_cnt = 0;
...
#if UART_S_UPDATE_CAPTRUE
bool deal_update_uart_start_msg(void)
{
    u8 ch = 0;
    while (upd_uart_isr_get(&ch)) {
        if (ch == upd_s_start_sign[upd_s_cnt]){
            upd_s_cnt++;
        } else {
            upd_s_cnt=0;
        }
        if (upd_s_cnt == sizeof(upd_s_start_sign)){
            upd_s_cnt = 0;
            return true;
        }
    }
...
{
    if (get_uart_upd_step() == UPD_IDEL) {
        if (deal_update_uart_start_msg()) {
            uart_send_recive_success();
            set_uart_upd_step(UPD_START);
            return true;
        } else {
            return false;
        }
