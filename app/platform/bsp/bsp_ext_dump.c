...
#define HUART_DUMP_BAUD          2000000 // 9600 //波特率使用2000000
...
void huart_module_init(void)   //huart DMA输出测试  //测试发现huart暂时只支持26M时钟
{
    ...
    //接收部分
    HSUT0CON |= (BIT(4) | BIT(2));// RX DMA MODE
    HSUT0CON |= BIT(0);         // RX EN
    HSUT0RXADR = (u32)RX_BUF;
    HSUT0RXCNT = 8;//此数值需与RX_BUF的长度一致
